	...
	if(server->nb_free_threads < 1) {
		...
		if(server->state >= SERVER_STARTING) {
			...
			if(node)
				...
				liblock_thread_create_and_bind(server->core, "eercl", &elected->tid, &attr, servicing_thread, elected);
...
static void* manager_thread(void* arg) {
	...
	liblock_thread_create_and_bind(server->core, "eercl", &backup_tid, &attr, backup_thread, server);
...
static void launch_server(struct server* server, void (*callback)()) {
	...
	liblock_thread_create_and_bind(server->core, "eercl", &tid, 0, manager_thread, server);
...
static struct liblock_impl* do_liblock_init_lock(eercl)(liblock_lock_t* lock, struct core* core, pthread_mutexattr_t* attr) {
	...
	liblock_reserve_core_for(core, "eercl");
...
static void do_liblock_run(eercl)(void (*callback)()) {
	...
	for(i=0; i<topology->nb_cores; i++) {
		if(topology->cores[i].server_type && !strcmp(topology->cores[i].server_type, "eercl"))
			n++;
	}
...
static void do_liblock_declare_server(eercl)(struct core* core) {
