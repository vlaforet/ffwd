 ...
 * Description:
 *   Concurrent lock-based accesses of the hashtable
 *
...
#ifdef SSYNC
#include "hashtable-lock_ssync.h"
#elif FFWD
#include <stdint.h>
#include "ffwd.h"
...
#ifdef FFWD
 	GET_CONTEXT()
	thread_data_t *d = (thread_data_t *)(context->initvalue);
#elif SSYNC
	...
	while (AO_load_full(&stop) == 0) {
		...
		if (unext) { // update
...
#ifdef SSYNC
				if (ht_add(d->set, val, TRANSACTIONAL, &client_data1, &client_data2)) 
#else
			...
			{ // remove
				...
				{ // alternate mode
#ifdef SSYNC
					if (ht_remove(d->set, last, TRANSACTIONAL, &client_data1, &client_data2)) 
#else
...
#ifdef SSYNC
					if (ht_remove(d->set, val, TRANSACTIONAL, &client_data1, &client_data2)) 
#else
...
#ifdef SSYNC
				if (ht_contains(d->set, val, TRANSACTIONAL, &client_data1, &client_data2)) 
#else
...
int main(int argc, char **argv)
{
  ...
  launch_servers(4);
...
#elif FFWD
		ffwd_thread_create(&threads[i], NULL, test, (void *)(&data[i]));
#endif
	}
	...
	/* To show output in Mops */
	//duration *= ONE_MILLION;
...
#if 0
	printf("#txs          : %lu (%f op / s)\n", reads + updates , (reads + updates) * 1000.0 / duration);
	
	printf("#read txs     : ");
	if (effective) {
		printf("%lu (%f op / s)\n", effreads, effreads * 1000.0 / duration);
		printf("  #contains   : %lu (%f op / s)\n", reads, reads * 1000.0 / duration);
	} else printf("%lu (%f op / s)\n", reads, reads * 1000.0 / duration);
	...
	if (effective) {
		...
		printf("  #upd trials : %lu (%f op / s)\n", updates, updates * 1000.0 / 
					 duration);
	} else printf("%lu (%f op / s)\n", updates, updates * 1000.0 / duration);
	
	printf("#aborts       : %lu (%f Mop / s)\n", aborts, aborts * 1000.0 / duration);
	printf("  #lock-r     : %lu (%f Mop / s)\n", aborts_locked_read, aborts_locked_read * 1000.0 / duration);
	printf("  #lock-w     : %lu (%f Mop / s)\n", aborts_locked_write, aborts_locked_write * 1000.0 / duration);
	printf("  #val-r      : %lu (%f Mop / s)\n", aborts_validate_read, aborts_validate_read * 1000.0 / duration);
	printf("  #val-w      : %lu (%f Mop / s)\n", aborts_validate_write, aborts_validate_write * 1000.0 / duration);
	printf("  #val-c      : %lu (%f Mop / s)\n", aborts_validate_commit, aborts_validate_commit * 1000.0 / duration);
	printf("  #inv-mem    : %lu (%f Mop / s)\n", aborts_invalid_memory, aborts_invalid_memory * 1000.0 / duration);
	printf("Max retries   : %lu\n", max_retries);
#endif
	/* Delete set */
	ht_delete(set);
